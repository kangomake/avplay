...
- (NSString *)getNowCellText{
    ...
    NSInteger index = self.mainScrollView.contentOffset.y / self.height;
    if(index < self.totalItemCount){
        return self.textArr[[self currentIndexWithRow:index]];;
    }else{
        return [self.textArr firstObject];
    }
    return @"";
}
